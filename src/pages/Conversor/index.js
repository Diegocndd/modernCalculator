...
const Conversor = () => {
      ...
      measures.push(
        <TouchableOpacity
          ...
          style={[
            ...
            {
              marginRight: i == data.length - 1 ? 0 : 30,
              borderColor: pressed == i ? '#fff' : 'transparent',
            },
            {width: selectedMeasure === 4 ? 50 : 40},
          ]}>
          <Text style={styles.textMeasure}>{data[i]}</Text>
        </TouchableOpacity>,
      );
    }
    return measures;
  ...
  const renderFinalMeasures = type => {
    ...
    for (let i = 0; i < data.length; i++) {
      measures.push(
        <TouchableOpacity
          onPress={() => {setFinalPressed(i)}}
          style={[
            styles.measureOption,
            {
              marginRight: i == data.length - 1 ? 0 : 30,
              borderColor: finalPressed == i ? '#fff' : 'transparent',
            },
            {width: selectedMeasure === 4 ? 50 : 40},
          ]}>
          <Text style={styles.textMeasure}>{data[i]}</Text>
    ...
    <View>
      <View style={styles.firstComponent}>
        <View
          ...
          onTouchStart={() => setHideInput(true)}>
          <DropDownPicker
            items={data}
